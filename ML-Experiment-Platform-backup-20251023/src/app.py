...
def get_task_type_extended(model_name):
    ...
    classification = {"逻辑回归", "支持向量机(SVM)", "K近邻(KNN)", "决策树", "随机森林", "梯度提升树(GBDT)", "多层感知机(MLP)", "朴素贝叶斯", "线性判别分析(LDA)", "深度神经网络(DNN)", "卷积神经网络(CNN)", "循环神经网络(RNN)", "长短期记忆网络(LSTM)"}
    regression = {"线性回归", "岭回归", "Lasso回归"}
