...
import pandas as pd
import numpy as np
from sklearn.datasets import load_iris, load_wine, load_breast_cancer, load_digits, make_classification, make_regression, load_diabetes, load_linnerud, fetch_california_housing, fetch_openml
...
class SVMModel(SVC):
    def __init__(self, C=1.0, gamma='scale', kernel='rbf', random_state=42, probability=True):
        super().__init__(C=C, gamma=gamma, kernel=kernel, random_state=random_state, probability=probability)
...
class RandomForestModel(RandomForestClassifier):
    def __init__(self, n_estimators=100, max_depth=None, random_state=42):
        super().__init__(n_estimators=n_estimators, max_depth=max_depth, random_state=random_state)
...
@register_model("LightGBM分类", "classification", "LightGBM梯度提升分类模型")
...
class MLPModel(MLPClassifier):
    def __init__(self, hidden_layer_sizes=(100,), learning_rate_init=0.001, max_iter=1000, random_state=42):
        super().__init__(hidden_layer_sizes=hidden_layer_sizes, learning_rate_init=learning_rate_init, 
...
@register_model("LightGBM回归", "regression", "LightGBM梯度提升回归模型")
...
class DNNModel(MLPClassifier):
    def __init__(self, hidden_layer_sizes=(100, 50, 25), learning_rate_init=0.001, max_iter=1000, random_state=42):
        super().__init__(hidden_layer_sizes=hidden_layer_sizes, learning_rate_init=learning_rate_init, 
                        max_iter=max_iter, random_state=random_state)

@register_model("卷积神经网络(CNN)", "classification", "卷积神经网络")
class CNNModel:
    def __init__(self, hidden_layer_sizes=(100, 50), learning_rate_init=0.001, max_iter=1000, random_state=42):
        # 简化的CNN实现，用于表格数据
        from sklearn.neural_network import MLPClassifier
        self.model = MLPClassifier(hidden_layer_sizes=hidden_layer_sizes, learning_rate_init=learning_rate_init, 
                                 max_iter=max_iter, random_state=random_state)
...
class RNNModel:
    def __init__(self, hidden_layer_sizes=(50, 25), learning_rate_init=0.001, max_iter=1000, random_state=42):
        # 简化的RNN实现，用于表格数据
        from sklearn.neural_network import MLPClassifier
        self.model = MLPClassifier(hidden_layer_sizes=hidden_layer_sizes, learning_rate_init=learning_rate_init, 
                                 max_iter=max_iter, random_state=random_state)
...
class LSTMModel:
    def __init__(self, hidden_layer_sizes=(64, 32), learning_rate_init=0.001, max_iter=1000, random_state=42):
        # 简化的LSTM实现，用于表格数据
        from sklearn.neural_network import MLPClassifier
        self.model = MLPClassifier(hidden_layer_sizes=hidden_layer_sizes, learning_rate_init=learning_rate_init, 
                                 max_iter=max_iter, random_state=random_state)
...
def load_dataset(dataset_name, test_size=0.3):
    if dataset_name == "iris":
        ...
        # 波士顿房价数据集 (已移除，使用替代数据集)
        # 注意：load_boston已被scikit-learn移除，这里使用合成数据作为替代
        ...
        # CIFAR-10数据集 (适合CNN，但需要特殊处理)
        try:
            # 使用OpenML的CIFAR-10数据集
        ...
        # IMDB电影评论数据集 (适合RNN/LSTM)
        try:
            # 使用OpenML的IMDB数据集
            data = fetch_openml('IMDB_reviews', version=1, as_frame=False)
            X = data.data
            y = data.target.astype(int)
            feature_names = [f'feature_{i}' for i in range(X.shape[1])]
        ...
        # 泰坦尼克号数据集
        X, y, feature_names, target_names = create_titanic_dataset()
        return X, y, feature_names, target_names, None
        ...
        try:
            # 尝试不同的文件路径
            possible_paths = [
                'assets/datasets/watermelon.csv',
...
def create_titanic_dataset():
    ...
    # 船舱等级 (Pclass): 1-3，均匀分布
    pclass = np.random.choice([1, 2, 3], n_samples, p=[0.2, 0.3, 0.5])
    
    # 登船港口 (Embarked): 0-2，分类变量
    embarked = np.random.choice([0, 1, 2], n_samples, p=[0.7, 0.2, 0.1])
    
    # 组合特征
    X = np.column_stack([age, fare, sibsp, parch, sex, pclass, embarked])
    
    # 生成目标变量（生存率）
    # 基于年龄、性别、船舱等级等特征计算生存概率
    survival_prob = (
        0.3 +  # 基础生存率
        0.2 * (sex == 1) +  # 女性生存率更高
        0.1 * (pclass == 1) +  # 一等舱生存率更高
        0.05 * (age < 18) +  # 儿童生存率更高
        -0.1 * (age > 60) +  # 老人生存率更低
        -0.05 * (fare < 10)  # 低票价生存率更低
    )
    survival_prob = np.clip(survival_prob, 0, 1)
    y = np.random.binomial(1, survival_prob, n_samples)
    
    feature_names = ['Age', 'Fare', 'SibSp', 'Parch', 'Sex', 'Pclass', 'Embarked']
...
def get_feature_importance(model, n_features, feature_names=None, X=None, y=None, task_type='classification'):
    ...
    # 如果所有方法都失败，返回平均分布
    n_features = X.shape[1] if hasattr(X, 'shape') else len(X.columns) if hasattr(X, 'columns') else len(X[0]) if X else 0
...
def train_and_evaluate(model_name, X, y, test_size, split_method, metric='accuracy', hyperparams=None, preprocessing_config=None, dataset_name='unknown', cv_folds='5', original_feature_names=None):
    ...
    if hyperparams and hyperparams.get('enable_tuning', False):
        ...
        if split_method in ['kfold', 'stratified_kfold']:
            # 如果已经是交叉验证，超参数调优使用较少的折数避免过度验证
            tuning_cv = 3  # 使用3折进行超参数调优
            send_progress(27, f'检测到交叉验证模式，超参数调优使用{tuning_cv}折避免过度验证...')
        else:
            # 普通分割模式，使用5折进行超参数调优
            tuning_cv = 5
            send_progress(27, f'普通分割模式，超参数调优使用{tuning_cv}折交叉验证...')
        
        tuning_result = hyperparameter_tuning(model, model_name, X_processed, y, param_grid, cv=tuning_cv, search_type=search_type)
        if tuning_result:
            ...
            # 超参数调优失败，记录错误信息
            results['hyperparameter_tuning'] = {
                'error': '超参数调优失败，使用默认参数',
                'search_type': search_type,
                'cv_folds': tuning_cv
            }
            results['warnings'].append('超参数调优失败，使用默认参数继续训练')
            send_progress(35, '超参数调优失败，使用默认参数继续训练')
    ...
    # 为深度学习模型生成特殊的可视化数据
    if model_name in ['深度神经网络(DNN)', '卷积神经网络(CNN)', '循环神经网络(RNN)', '长短期记忆网络(LSTM)']:
        deep_learning_viz = get_deep_learning_visualization(model_name, model, X_processed, y, get_task_type_extended(model_name))
        results['deep_learning_visualization'] = deep_learning_viz
        results['feature_importance'] = deep_learning_viz['feature_importance']
    else:
        results['feature_importance'] = get_feature_importance(model, n_features, feature_names, X_processed, y, get_task_type_extended(model_name))
...
def get_model_type(model_name):
    """获取模型类型"""
    tree_based = ["决策树", "随机森林", "梯度提升树(GBDT)", "LightGBM分类", "LightGBM回归"]
    linear_models = ["逻辑回归", "线性回归", "岭回归", "Lasso回归"]
    neural_models = ["多层感知机(MLP)", "深度神经网络(DNN)", "卷积神经网络(CNN)", "循环神经网络(RNN)", "长短期记忆网络(LSTM)"]
    distance_based = ["K近邻(KNN)"]
    probabilistic = ["朴素贝叶斯"]
    kernel_based = ["支持向量机(SVM)", "支持向量回归(SVR)"]
    ensemble_models = []
    unsupervised_models = ["K 均值聚类(K-Means)", "主成分分析(PCA)"]
    discriminant_models = ["线性判别分析(LDA)"]
    ...
    if model_name in tree_based:
        ...
        return "无监督学习"
    elif model_name in discriminant_models:
        return "判别分析"
    else:
        return "其他"
...
def get_task_type_extended(model_name):
    ...
    classification = {"逻辑回归", "支持向量机(SVM)", "K近邻(KNN)", "决策树", "随机森林", "梯度提升树(GBDT)", "LightGBM分类", "多层感知机(MLP)", "朴素贝叶斯", "线性判别分析(LDA)", "深度神经网络(DNN)", "卷积神经网络(CNN)", "循环神经网络(RNN)", "长短期记忆网络(LSTM)"}
    regression = {"线性回归", "岭回归", "Lasso回归", "LightGBM回归", "支持向量回归(SVR)"}
...
def get_dataset_type(dataset_name):
    ...
    classification_datasets = {"iris", "wine", "breast_cancer", "digits", "titanic", "fashion_mnist", "cifar10", "imdb_reviews"}
    regression_datasets = {"boston", "diabetes", "california_housing", "linnerud"}
    unsupervised_datasets = {"blobs", "circles", "moons", "anisotropic"}
